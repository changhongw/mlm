...
class Transformer(Module):
    ...
    def __init__(self, 
        ...
        # get embedding
        self.map_music = nn.Linear(music_in, d_model)
        self.map_lyric = nn.Linear(lyric_in, d_model) 

        encoder_norm_m = LayerNorm(d_model)
        encoder_norm_l = LayerNorm(d_model)

        ########## music encoder ##########
        encoder_layer_m = TransformerEncoderLayerRPR(d_model=d_model,
                                                nhead=nhead,
                                                dim_feedforward=dim_ffd,
                                                dropout=dropout,
                                                er_len=er_len)

        self.encoder_m = TransformerEncoderRPR(encoder_layer_m, num_layers, encoder_norm_m)

        ########## lyrics encoder ##########
        encoder_layer_l = TransformerEncoderLayerRPR(d_model=d_model,
                                                nhead=nhead,
                                                dim_feedforward=dim_ffd,
                                                dropout=dropout,
                                                er_len=er_len)

        self.encoder_l = TransformerEncoderRPR(encoder_layer_l, num_layers, encoder_norm_l)

        ########## projector ##########
        self.probe_m = Projection(d_model, d_model, dropout=0.5)
        self.probe_l = Projection(d_model, d_model, dropout=0.5)
    ...
    def forward(self, x, y, x_mask, y_mask):
        ...
        xe = self.encoder_m(xe.transpose(0,1), src_key_padding_mask=x_mask)
        ye = self.encoder_l(ye.transpose(0,1), src_key_padding_mask=y_mask)

        x_proj = self.probe_m(xe.transpose(0,1))
        y_proj = self.probe_l(ye.transpose(0,1))
...
class MultiheadAttentionRPR(Module):
    ...
    def __init__(self, embed_dim, num_heads, dropout=0., bias=True, add_bias_kv=False, add_zero_attn=False, kdim=None, vdim=None, er_len=None):
        ...
        # Adding RPR embedding matrix                norm=None
    ...
    def _reset_parameters(self):
        ...
        if(self.er_len is not None):
            xavier_uniform_(self.Er)
...
def multi_head_attention_forward_rpr(query,                       # type: Tensor
    ...
    if(rpr_mat is not None):
        ...
        qe = torch.einsum("hld,md->hlm", q, rpr_mat) # tensor contraction
        srel = _skew(qe)
